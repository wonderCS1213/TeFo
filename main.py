...
class Runner:
    ...
    def model_choise(self):
        assert self.args.model_name in ["EVA", "MCLEA", "MSNEA", "TeFo"]
        if self.args.model_name == "TeFo":
            self.model = TeFo(self.KGs, self.args)
    ...
    def il_for_ea(self):
        with torch.no_grad():
            if self.args.model_name in ["TeFo"]:
