...
from .TeFo_tools import MultiModalEncoder
from .TeFo_loss import CustomMultiLossLayer, icl_loss

from src.utils import pairwise_distances
import os.path as osp
import json

class GatingCompressor(nn.Module):
    def __init__(self, input_dim):
        super(GatingCompressor, self).__init__()
...
class ModalityDiscriminator(nn.Module):
    def __init__(self, input_dim, num_modalities=6):
...
class TeFo(nn.Module):
    def __init__(self, kgs, args):
        ...
        self.use_adv = getattr(args, "use_adv", True)

        if self.use_adv:
            emb_dim = self.multimodal_encoder.input_dim
            self.modality_disc = ModalityDiscriminator(input_dim=emb_dim, num_modalities=self.args.inner_view_num)
            self.alpha_adv = getattr(args, "alpha_adv", 1)
            
            self.gate = GatingCompressor(input_dim=emb_dim)



    def forward(self, batch):
        gph_emb, img_emb, rel_emb, att_emb, name_emb, char_emb, joint_emb, hidden_states, weight_norm = self.joint_emb_generat(only_joint=False)
        ...
        # === Step: Cross-Modal Self-Distillation with Adaptive Teacher Selection (Vectorized) ===
        if getattr(self.args, "use_distill1", True):
            distill_loss = 0
            print("adaptive teacher")
            modal_embs = [gph_emb, rel_emb, att_emb, img_emb, name_emb, char_emb]
            modal_embs = [emb for emb in modal_embs if emb is not None]  # 去掉 None
            modal_num = len(modal_embs)

            # 创建教师表征，batch级别，每个样本都是加权模态组合
            stacked_embs = torch.stack([F.normalize(e, dim=1) for e in modal_embs], dim=1)  # [B, modal_num, D]
            teacher_emb = torch.sum(stacked_embs * weight_norm.unsqueeze(-1), dim=1).detach()  # [B, D]

            # 学生为每个单独模态
            for student_emb in modal_embs:
                student_logp = F.log_softmax(student_emb, dim=1)
                teacher_p = F.softmax(teacher_emb, dim=1)
                distill_loss += F.kl_div(student_logp, teacher_p, reduction="batchmean")
         
            beta1 = 100
            print("beta1", beta1)

            loss_all = loss_all + beta1 * distill_loss
        ...
        if self.use_adv2:
            print("gating-adv")
            modality_emb_list = []
            #modality_emb_list = [gph_emb, rel_emb, att_emb, img_emb]
            if gph_emb is not None: modality_emb_list.append(gph_emb)
            if rel_emb is not None: modality_emb_list.append(rel_emb)
            if att_emb is not None: modality_emb_list.append(att_emb)
            if img_emb is not None: modality_emb_list.append(img_emb)
            if name_emb is not None: modality_emb_list.append(name_emb)
            if char_emb is not None: modality_emb_list.append(char_emb)

            z_all = []
            for emb in modality_emb_list:
               z = self.gate(emb)
               z_all.append(z)
            emb_all = torch.cat(z_all, dim=0)
            
            labels_all = torch.cat([
                torch.full((z.shape[0],), i, dtype=torch.long) for i, z in enumerate(z_all)
            ]).cuda()

            emb_all_rev = grad_reverse(emb_all, self.alpha_adv)
            logits = self.modality_disc(emb_all_rev)
            loss_adv = F.cross_entropy(logits, labels_all)
            print("self.alpha_adv", self.alpha_adv)
            loss_all = loss_all + self.alpha_adv * (loss_adv)
